...
def colorTran(source,target,number):
	...
	trMean = np.hstack(np.around(trMean, decimals=2))
	trStd = np.hstack(np.around(trStd, decimals=2))
	...
	for n in range(0,101):
		...
		iMean = np.hstack(np.around(iMean, decimals=2))
		iStd = np.hstack(np.around(iStd, decimals=2))
	...
	for i in range (0,height): 
		for j in range (0,width): 
			for k in range (0,channel): 
				s = sImg[i,j,k] 
				s = ((Weight_MIN*trStd[k]+(1-Weight_MIN)*sStd[k]) / sStd[k]) * (s-sMean[k]) + trMean[k]*Weight_MIN + (1-Weight_MIN)*sMean[k]
				s = round(s)
				if s < 0:
					s = 0
				if s > 255:
